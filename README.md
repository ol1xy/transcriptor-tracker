...
### Окружение Docker (изолированный запуск)

* `make docker-build` — сборка Docker-образа с предварительной автоматической проверкой и генерацией `.env` из шаблона.
* `make docker-up` — запуск основного контейнера для обработки аудио. Позволяет передавать внешние параметры.
* `make docker-down` — остановка активных контейнеров и очистка временных ресурсов Docker-композиции.
* `make docker-test` — запуск автотестов внутри изолированного контейнера.
* `make docker-lint` — статический анализ кода (flake8) силами контейнера.
* `make docker-coverage` — запуск тестов с расчетом покрытия кода внутри контейнера.

---

## 6. Контейнеризация и изоляция среды выполнения (Docker)

Для устранения различий в окружении разработчиков, а также для упрощения развертывания на различных операционных системах (включая Windows без WSL), в проект добавлена поддержка Docker и Docker Compose. 

Это исключает необходимость локальной установки Python-зависимостей и системных библиотек обработки звука (например, `ffmpeg`).

### Специфика реализации:

1. **Легковесный базовый образ:** Сборка базируется на образе `python:3.11-slim`, содержащем необходимый минимум для работы транскрибатора. В системное окружение добавлен пакет `ffmpeg`.
2. **Безопасность выполнения (Non-root):** Приложение внутри контейнера выполняется под учетной записью ограниченного несистемного пользователя `appuser`. Процессу не выдаются права суперпользователя (root), что минимизирует риски безопасности.
3. **Кэширование моделей распознавания:** Веса моделей Whisper сохраняются во внешнем именованном томе Docker (`whisper_models`), примонтированном к `/app/models_cache`. Это предотвращает повторное скачивание файлов моделей при повторных запусках контейнера.
4. **Сквозная работа с данными:** Папка проекта `data/` на хост-машине монтируется напрямую в контейнер. Все входные аудиозаписи считываются из нее, а сгенерированные артефакты (`transcript.md`, `result.json`, `evidence-event.json`) сохраняются локально на диск пользователя сразу после завершения обработки.
5. **Предохранитель авторизации:** В Makefile интегрирована цель создания конфигурационного файла `.env`. При первом запуске сборки или контейнера система проверяет наличие `.env`, автоматически создает его копию из `.env.example` и останавливает процесс выполнения с кодом возврата `1`, напоминая пользователю о необходимости внести `GEMINI_API_KEY`.

--- 

### Пример параметризованного запуска в Docker:

Для анализа произвольного аудиофайла с указанием конкретных идентификаторов проекта и задачи выполните:

```bash
make docker-up DOCKER_AUDIO=data/examples/sample-meeting.mp3 DOCKER_PROJECT_ID=my-project DOCKER_ISSUE_ID=42
```
